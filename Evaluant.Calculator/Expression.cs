...
namespace NCalc
{
    public class Expression
    {
        ...
        public object Evaluate()
        {
            ...
            catch (RawExpressionException e)
            {
                return EvaluateRaw();
            }
        ...
        {
            RawExpressionVisitor visitor = new RawExpressionVisitor(Options);
            visitor.ValidateFunction += ValidateFunction;
            visitor.EvaluateFunction += EvaluateFunction;
            visitor.EvaluateParameter += EvaluateParameter;
            visitor.Parameters = Parameters;
            ParsedExpression.Accept(visitor);
            string expression = visitor.Result.ToString();
            // check if the resulting expresison is a variable or not
            if (expression[0] == '[' && expression[expression.Length - 1] == ']')
            {
                return new RawIdentifierExpression(expression.Substring(1, expression.Length - 2));
            }
            else
            {
                return new RawExpression(visitor.Result.ToString());
            }
        }
